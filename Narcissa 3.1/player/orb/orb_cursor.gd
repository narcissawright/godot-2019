...
var cursor_tex = load("res://player/orb/orb_crosshair.png")
var target_list:Array = []  # assigned in orb.gd
...
func _draw(): # update called in orb.gd
	for i in range (target_list.size()):
		if target_list[i].blocked:
			continue # if the orb has no direct line of sight to the target, do not draw crosshair
		...
		for i in range (points.size()):
			points[i] = Game.cam.unproject_position(points[i])
			
		# sort the eight 2d points by their x position
		var x_sort = []
		for i in range (points.size()):
			x_sort.append(points[i].x)
		x_sort.sort()
		
		# sort by y position
		var y_sort = []
		for i in range (points.size()):
			y_sort.append(points[i].y)
		y_sort.sort()
